...
export function ForgotPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Em uma aplicação real, você enviaria um email com este código
      // Para esta demonstração, vamos armazenar o código no localStorage
      const resetRequests = JSON.parse(localStorage.getItem("resetRequests") || "{}")
      resetRequests[email] = {
        code: verificationCode,
        expiresAt: new Date(Date.now() + 30 * 60 * 1000).toISOString(), // Expira em 30 minutos
      }
      ...
      // Em uma aplicação real, você enviaria um email com um link
      // Para esta demonstração, vamos redirecionar para a página de redefinição após um delay
      setTimeout(() => {
